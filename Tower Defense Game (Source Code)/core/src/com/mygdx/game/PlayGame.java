...
public class PlayGame extends BaseScreen  {
	...
	Animation enemyAnim1,enemyAnim2,enemyAnim3;
	...
	int maxattacktower=3,maxdefensetower=50,maxauratower=1;
	...
	public void create () {
		 ...
		 attackTower.addListener(new InputListener(){
			 public boolean touchDown(InputEvent ev,float x,float y,int pointer,int button){
				if(kulekur && attacknum<maxattacktower){
						Tower t=new SingleTower(enemies, 100);
		...
		//mainStage.addActor(background);
		 defenseTower.addListener(new InputListener(){
			 public boolean touchDown(InputEvent ev,float x,float y,int pointer,int button){
				if(kulekur && defensenum<maxdefensetower){
						Tower t=new DefenceTower(enemies, 700);
		 ...
		 auraTower.addListener(new InputListener(){
			 public boolean touchDown(InputEvent ev,float x,float y,int pointer,int button){
				 if(kulekur && auranum<maxauratower){
						Tower t=new SlowTower(enemies, 150);
		 ...
		 startButton.addListener(new InputListener(){
			 public boolean touchDown(InputEvent ev,float x,float y,int pointer,int button){
				 ...
				 enemyrender-=30;
				 	...
				 	sendenemy=true;
					up();
					  System.out.println(foundpath1);
					  System.out.println(foundpath2);
					  System.out.println(foundpath3);
					  System.out.println(foundpath4);
		 ...
		 uiTable.row();
		 uiTable.add(towerMenu).colspan(3).padTop(-100).padRight(-510);
		 uiTable.top();
	...
	public void render(float delta) {
		...
		for(Tower t:towers){
			...
			Rectangle rec=new Rectangle(squareX, squareY, towerWidth,towerHeight);
			if(!(Math.abs(squareX-level.getX())<70 && Math.abs(squareY-level.getY())<40))
				if(!(Math.abs(squareX-score.getX())<70 && Math.abs(squareY-score.getY())<40)){
					drawsquare=true;
			shape.rect(squareX, squareY, towerWidth, towerHeight);
			shape.end();*/
	...
	public void update(float dt) {
		...
		for(enemy e:enemies){
		 ...
		 if(enemies.size()>0)
			 for(enemy e:enemies)
				 if(e.visible ||(enemies.size()%controlenemy!=0))
					 c=false;
		 if(!c){
			 ennumber=0;
			 attacknum=0;
