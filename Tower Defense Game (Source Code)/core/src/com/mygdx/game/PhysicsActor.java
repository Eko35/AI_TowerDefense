...
public class PhysicsActor extends AnimatedActor {
	...
	public void setVelocities(enemy hedef,float speed){
		if(getDistanceX(hedef)+hedef.velocityX <0)
			velocity.x=Math.max(-speed,getDistanceX(hedef)+hedef.velocityX);
		else
			velocity.x=Math.min(speed,getDistanceX(hedef) +hedef.velocityX);
		
		if(getDistanceY(hedef)+hedef.velocityY <0)
			velocity.y=Math.max(-speed,getDistanceY(hedef)+hedef.velocityY);
		else
			velocity.y=Math.min(speed,getDistanceY(hedef)+hedef.velocityY );
	}
